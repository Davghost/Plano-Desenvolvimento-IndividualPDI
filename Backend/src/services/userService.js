...
async function UpdatePDIService(id_user, data) {

    if (!Array.isArray(data.pdiItems)) {
        const error = new Error("pdiItems deve ser um array.");
    ...
    for (const item of data.pdiItems) {

        try {

            if (!item.theme) {
                throw new Error("Theme é obrigatório.");
            }

            const parsed = PdiSchemas.updatePDISchema.safeParse({
                objective: item.objective,
                why: item.why,
                how: item.how,
                period: item.period,
